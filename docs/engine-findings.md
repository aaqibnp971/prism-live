...
- **Keep one writer.** On `a8ad35f`, the host's override and the engine's inference thread can publish at once, a race that `dbf0f24` fixed upstream (its commit message: "not a click, a quietly wrong room"). Even on `acbfd50` it is simplest for the host to be the only PSV writer. The code shows `prism_render` needs only a loaded scene, not `prism_start`, so one way is never to start inference. Verify in prompt 2.5. *Verified 14 September (prompt 2.5, `tests/test_engine.py`): with the committed acbfd50 DLL and no `prism_start`, `prism_render` renders a loaded scene and `prism_set_mood_override` is honoured. A dark override against a bright one moves the 2 to 10 kHz energy by 54 dB, the low-pass lands within 0.1 dB of the cutoff the mapping gives, and the pulse gate opens and closes on its loop boundary.* A pin set before `prism_start` also reuses sequence number 1 when `prism_start` publishes.
